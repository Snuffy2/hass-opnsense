...
async def test_async_setup_entry_creates_entities_when_enabled(make_config_entry):
    ...
    # enable both sync options (CARP flag should be ignored by binary_sensor setup)
...
@pytest.mark.asyncio
async def test_async_setup_entry_creates_no_binary_entities_when_only_carp_enabled(
    make_config_entry,
):
    """Create no binary entities when only CARP sync is enabled."""
    # enable only CARP; CARP binary sensor was removed
    entry = make_config_entry(
