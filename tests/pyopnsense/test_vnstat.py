...
@pytest.mark.asyncio
async def test_get_vnstat_skips_calls_when_plugin_missing(make_client) -> None:
    """get_vnstat should return empty payload and skip API calls when plugin is absent."""
    session = MagicMock(spec=aiohttp.ClientSession)
    client = make_client(session=session)
    try:
        ...
        client.is_named_plugin_installed.assert_awaited_once_with("os-vnstat")
    finally:
        await client.async_close()
