...
@pytest.mark.asyncio
async def test_isc_dhcp_plugin_not_installed(make_client) -> None:
    """ISC DHCP lease methods should return empty list when plugin is not installed."""
    session = MagicMock(spec=aiohttp.ClientSession)
    client = make_client(session=session)
    try:
        client.is_named_plugin_installed = AsyncMock(return_value=False)
        client._safe_dict_get = AsyncMock()
...
async def test_get_isc_dhcpv4_and_v6_parsing() -> None:
    ...
    try:
        ...
        client.is_named_plugin_installed = AsyncMock(return_value=True)
        client._safe_dict_get = AsyncMock(
            side_effect=[
                {
                    "rows": [
...
async def test_get_dhcp_leases_combined_structure() -> None:
    ...
    try:
        ...
        client.is_named_plugin_installed.assert_awaited_once_with("os-isc-dhcp")
    finally:
        await client.async_close()


@pytest.mark.asyncio
async def test_get_dhcp_leases_skips_isc_when_plugin_missing() -> None:
    """get_dhcp_leases should skip ISC endpoints entirely when plugin is absent."""
    session = MagicMock(spec=aiohttp.ClientSession)
    client = pyopnsense.OPNsenseClient(
        url="http://localhost", username="u", password="p", session=session
    )
    try:
        ...
        assert "em0" in combined["leases"]
        client._get_isc_dhcpv4_leases.assert_not_awaited()
        client._get_isc_dhcpv6_leases.assert_not_awaited()
        client.is_named_plugin_installed.assert_awaited_once_with("os-isc-dhcp")
    finally:
        await client.async_close()
