...
@pytest.mark.asyncio
async def test_add_plugin_compat_caches_empty_direct_key_payload() -> None:
    """Plugin compatibility shim should cache direct-key payloads without target plugin."""

    class _Client:
        def __init__(self) -> None:
            """Initialize fake client with direct-key firmware payload."""
            self._safe_dict_get = AsyncMock(return_value={"os-vnstat": "1"})
