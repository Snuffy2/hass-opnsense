...
class OPNsenseOptionsFlow(OptionsFlow):
    ...
    async def async_step_init(
        ...
        if user_input is not None:
            ...
            if self._config.get(CONF_DEVICE_TRACKER_ENABLED):
            ...
            return self.async_create_entry(title="", data=self._config, options=self._options)
    ...
    async def async_step_device_tracker(
        ...
        if user_input is not None:
            ...
            self._options.update(user_input)
            _LOGGER.debug("[options_flow device_tracker] merged options: %s", self._options)
            macs: list = []
    ...
    async def async_step_granular_sync(
        ...
        if user_input is not None:
            ...
            self._config.update(user_input)
            _LOGGER.debug("[options_flow granular_sync] merged config: %s", self._config)
            self._errors = await validate_input(
                hass=self.hass, user_input=self._config, errors=self._errors
            )
            if not self._errors:
