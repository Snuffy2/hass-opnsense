    ...
    return vol.Schema(
        {
            vol.Optional(
                CONF_SCAN_INTERVAL,
                default=user_input.get(
                    CONF_SCAN_INTERVAL, fallback.get(CONF_SCAN_INTERVAL, DEFAULT_SCAN_INTERVAL)
                ),
            ...
            vol.Optional(
                ...
                default=user_input.get(
                    CONF_DEVICE_TRACKER_ENABLED,
                    fallback.get(CONF_DEVICE_TRACKER_ENABLED, DEFAULT_DEVICE_TRACKER_ENABLED),
                ),
            ): selector.BooleanSelector(selector.BooleanSelectorConfig()),
            vol.Optional(
                CONF_DEVICE_TRACKER_SCAN_INTERVAL,
...
class OPNsenseConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_granular_sync(
        ...
        if user_input is not None:
            ...
            self._errors = await validate_input(
                hass=self.hass, user_input=self._user_input, errors=self._errors
            )
            if not self._errors:
                return self.async_create_entry(
    ...
    async def async_step_reconfigure(
        ...
        self._user_input = dict(reconfigure_entry.data)
        firmware: str = "Unknown"
        if user_input is not None:
            self._user_input.update(user_input)
            self._errors = await validate_input(
            ...
            firmware = self._user_input.get(CONF_FIRMWARE_VERSION, "Unknown")
            if not self._errors:
                # https://developers.home-assistant.io/docs/config_entries_config_flow_handler#unique-ids
                await self.async_set_unique_id(self._user_input.get(CONF_DEVICE_UNIQUE_ID))
        ...
        return self.async_show_form(
            ...
            data_schema=_build_user_input_schema(
                user_input=user_input, fallback=self._user_input, reconf=True
            ),
            errors=self._errors,
            description_placeholders={
                "firmware": firmware,
...
class OPNsenseOptionsFlow(OptionsFlow):
    ...
    async def async_step_init(
        ...
        if user_input is not None:
            ...
            _LOGGER.debug("[options_flow init] merged user_input: %s", self._user_input)
            if self._user_input.get(CONF_GRANULAR_SYNC_OPTIONS):
                return await self.async_step_granular_sync()
            for item in GRANULAR_SYNC_ITEMS:
                self._user_input.pop(item, None)
            if self._user_input.get(CONF_DEVICE_TRACKER_ENABLED):
                return await self.async_step_device_tracker()
            _LOGGER.debug("Updating options from init. user_input: %s", self._user_input)

            self.hass.config_entries.async_update_entry(
                entry=self.config_entry, data=self._user_input
            )
        ...
        return self.async_show_form(
            ...
            data_schema=_build_options_init_schema(
                user_input=user_input, fallback=self._user_input
            ),
        )
    ...
    async def async_step_device_tracker(
        ...
        if user_input is not None:
            ...
            self._user_input.update(user_input)
            _LOGGER.debug("[options_flow device_tracker] merged user_input: %s", self._user_input)
            macs: list = []
            manual_devices: str | None = self._user_input.pop(CONF_MANUAL_DEVICES, None)
            ...
            self._user_input[CONF_DEVICES] = user_input.get(CONF_DEVICES, []) + macs
            self.hass.config_entries.async_update_entry(
                entry=self.config_entry, data=self._user_input
            )
            return self.async_create_entry(data=self._user_input)
    ...
    async def async_step_granular_sync(
        ...
        if user_input is not None:
            ...
            self._user_input.update(user_input)
            _LOGGER.debug("[options_flow granular_sync] merged user_input: %s", self._user_input)
            self._errors = await validate_input(
                hass=self.hass, user_input=self._user_input, errors=self._errors
            )
            if not self._errors:
                if user_input.get(CONF_DEVICE_TRACKER_ENABLED):
                    return await self.async_step_device_tracker()
                _LOGGER.debug(
                    "Updating options from granular sync. user_input: %s", self._user_input
                )
