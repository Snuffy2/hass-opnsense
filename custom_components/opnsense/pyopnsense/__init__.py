    ...
    async def _process_queue(self) -> None:
        while True:
            ...
            try:
                ...
                _LOGGER.error(
                    "Exception in request queue processor (added by %s). %s: %s",
                    caller,
                    type(e).__name__,
                    e,
                )
            ...
            async with self._session.get(
                ...
                if response.ok:
                    ...
                    if response.status == 403:
                        _LOGGER.error(
                            "Permission Error in do_get_from_stream (added by %s). Path: %s. Ensure the OPNsense user connected to HA has full Admin access",
                            caller,
                            url,
                        )
                    else:
                        _LOGGER.error(
                            "Error in do_get_from_stream (added by %s). Path: %s. Response %s: %s",
                            caller,
                            url,
                            response.status,
                            response.reason,
    ...
    async def _do_get(
        ...
        try:
            async with self._session.get(
                ...
                if response.status == 403:
                    _LOGGER.error(
                        "Permission Error in do_get (added by %s). Path: %s. Ensure the OPNsense user connected to HA has full Admin access",
                        caller,
                        url,
                    )
                else:
                    _LOGGER.error(
                        "Error in do_get (added by %s). Path: %s. Response %s: %s",
                        caller,
                        url,
                        response.status,
                        response.reason,
    ...
    async def _do_post(
        ...
        try:
            async with self._session.post(
                ...
                if response.status == 403:
                    _LOGGER.error(
                        "Permission Error in do_post (added by %s). Path: %s. Ensure the OPNsense user connected to HA has full Admin access",
                        caller,
                        url,
                    )
                else:
                    _LOGGER.error(
                        "Error in do_post (added by %s). Path: %s. Response %s: %s",
                        caller,
                        url,
                        response.status,
                        response.reason,
